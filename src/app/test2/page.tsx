...
export default function Test2Page() {
  ...
  return (
    ...
    >
      ...
      />
      {/* Layer 1.5 — desktop sun (hidden on small screens) */}
      <svg
        ref={dskSunRef}
        viewBox={`0 0 ${W} ${H}`}
        preserveAspectRatio="xMidYMid slice"
        className="absolute inset-0 w-full h-full block pointer-events-none hidden md:block"
      />
      {/* Layer 2 — mobile-only corner sun (hidden on md and up) */}
      <svg
        ref={sunRef}
        viewBox="0 0 200 200"
        className="absolute top-3 left-3 sm:top-5 sm:left-5 w-20 sm:w-24 h-auto pointer-events-none md:hidden"
      />
      {/* Layer 3 — clouds (above both suns, below mid hills) */}
      <svg
        ref={cloudRef}
        viewBox={`0 0 ${W} ${H}`}
        preserveAspectRatio="xMidYMid slice"
      ...
      />
      {/* Layer 4 — mid hills + trees + ground + grass */}
      <svg
        ref={fgRef}
        viewBox={`0 0 ${W} ${H}`}
        preserveAspectRatio="xMidYMid slice"
...
function renderBackground(rootSvg: SVGSVGElement, rc: RoughSVG, t: number, boilTick: number) {
  ...
  // Sky — fills all the way to the ground so no stripe shows between sky and ground line
